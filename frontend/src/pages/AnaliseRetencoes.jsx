            ...
            {hist.length===0?(
              ...
              <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
                ...
                <tbody>
                  {hist.map((h,i)=>(
                    <tr key={h.id} style={{borderBottom:'1px solid #f0f0f0',background:i%2===0?'#fff':'#fafafa'}}>
                      <td style={{padding:'9px 12px',color:'#555'}}>{fmtData(h.data)}</td>
                      <td style={{padding:'9px 12px',fontWeight:600,color:NAVY}}>{h.empresa}</td>
                      <td style={{padding:'9px 12px',color:'#555'}}>{h.nota!=='-'?'NFS '+h.nota:'-'}</td>
                      <td style={{padding:'9px 12px',fontWeight:600}}>{fmtMoeda(h.valor)}</td>
                      <td style={{padding:'9px 12px'}}>
                        <div style={{display:'flex',gap:4,flexWrap:'wrap'}}>
                          {(h.retencoes_tags||[]).map(t=>(
                            <span key={t} style={{background:t==='INSS'?'#EBF5FF':t==='IRRF'?'#F3EEFF':t==='ISS'?'#EDFBF1':'#FFF3E0',color:t==='INSS'?'#1D6FA4':t==='IRRF'?'#6B3EC9':t==='ISS'?'#1A7A3C':'#E65100',padding:'2px 7px',borderRadius:12,fontSize:11,fontWeight:700}}>{t}</span>
                          ))}
                          {!(h.retencoes_tags||[]).length&&<span style={{color:'#aaa',fontSize:11}}>Nenhuma</span>}
                        </div>
                      </td>
                      <td style={{padding:'9px 12px'}}>
                        {(obrigGeradas[h.id]||h.obrig_geradas)>0?(
                          <span style={{background:'#d4edda',color:'#155724',padding:'2px 9px',borderRadius:12,fontSize:11,fontWeight:700}}>{obrigGeradas[h.id]||h.obrig_geradas} criadas</span>
                        ):<span style={{color:'#aaa',fontSize:11}}>-</span>}
                      </td>
                      <td style={{padding:'9px 12px'}}>
                        <div style={{display:'flex',gap:6}}>
                          <button onClick={()=>{setForm({...FORM0,...h.form});setResultado(h.resultado);window.scrollTo(0,0)}} style={{padding:'4px 9px',borderRadius:6,border:'1px solid '+NAVY,background:'#fff',color:NAVY,fontSize:11,cursor:'pointer',fontWeight:600}}>Ver</button>
                          {(h.retencoes_tags||[]).length>0&&!(obrigGeradas[h.id]||h.obrig_geradas)&&(
                            <button onClick={()=>gerarObrig(h)} style={{padding:'4px 9px',borderRadius:6,border:'none',background:'#28a745',color:'#fff',fontSize:11,cursor:'pointer',fontWeight:600,whiteSpace:'nowrap'}}>+ Obrigacoes</button>
                          )}
                        </div>
                      </td>
                    </tr>
