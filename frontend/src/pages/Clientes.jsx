...
export default function Clientes() {
  ...
  const onTributacaoChange = (novoRegime) => {
    ...
    setObrigSugeridas(todas); setF('obrigacoes_vinculadas', todas); setConfirmObrig(true)
  ...
  const gerarObrigacoes = () => {
    ...
    if (editId) {
      ...
      const updated = clisLocal.map(c => String(c.id)===String(editId) ? {...c, obrigacoes_vinculadas:todas, obrigacoes_catalogo:obrigsCatalogo(form.tributacao), tributacao:form.tributacao, regime:form.tributacao} : c)
  ...
  const confirmarObrigacoes = () => {
    ...
    if (editId) {
      const clisLocal = JSON.parse(localStorage.getItem('ep_clientes')||'[]')
      const updated = clisLocal.map(c => String(c.id)===String(editId) ? {...c, obrigacoes_vinculadas:form.obrigacoes_vinculadas} : c)
      localStorage.setItem('ep_clientes', JSON.stringify(updated)); setClientes(updated)
    }
  }
  ...
  const buscarCNPJ = async () => {
    ...
    try {
      ...
      if (r.ok) {
        const d = await r.json()
        setForm(f => ({ ...f, nome:d.razao_social||f.nome, nome_fantasia:d.nome_fantasia||f.nome_fantasia, logradouro:d.logradouro||f.logradouro, numero:d.numero||f.numero, complemento:d.complemento||f.complemento, bairro:d.bairro||f.bairro, cidade:d.municipio||f.cidade, estado:d.uf||f.estado, cep:d.cep||f.cep, data_abertura:d.data_inicio_atividade||f.data_abertura }))
      }
    } catch {}
  ...
  const salvar = async () => {
    const novoCliente = { ...form, id:editId||Date.now(), ativo:form.ativo!==false, obrigacoes_vinculadas:form.obrigacoes_vinculadas||[], obrigacoes_catalogo:form.obrigacoes_catalogo||[], responsaveis:form.responsaveis||[], contatos:form.contatos||[], tributacao:form.tributacao||'', regime:form.regime||form.tributacao||'' }
    let novaLista = []
    ...
    setForm({...FORM_VAZIO, responsaveis:[], contatos:[{nome:'',departamento:'',cargo:'',email:'',whatsapp:'',telefone:'',tipo:'principal'}]}); setEditId(null); setAba('lista')
    try { const metodo=editId?'PUT':'POST'; const url=editId?`${API}/clientes/${editId}`:`${API}/clientes/`; await fetch(url,{method:metodo,headers:{'Content-Type':'application/json'},body:JSON.stringify(novoCliente)}) } catch {}
  }

  const nova = () => { setForm({...FORM_VAZIO, responsaveis:[], contatos:[]}); setEditId(null); setAba('cadastro'); setAbaForm('dados') }
  const editar = (cli) => {
    setForm({ ...FORM_VAZIO, ...cli, obrigacoes_vinculadas:cli.obrigacoes_vinculadas||[], obrigacoes_catalogo:cli.obrigacoes_catalogo||[], responsaveis:cli.responsaveis||[], contatos:cli.contatos?.length?cli.contatos:[{nome:'',departamento:'',cargo:'',email:'',whatsapp:'',telefone:'',tipo:'principal'}], ativo:cli.ativo!==false, tributacao:cli.tributacao||cli.regime||'', regime:cli.regime||cli.tributacao||'' })
    setEditId(cli.id); setAba('cadastro'); setAbaForm('dados')
  }
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'calc(100vh - 44px)', fontFamily:'Inter, system-ui, sans-serif' }}>
            ...
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
              <thead>
                <tr style={{ background:'#fff', borderBottom:'2px solid #e8e8e8', position:'sticky', top:0, zIndex:1 }}>
                  {['Cliente','CNPJ','Regime','Responsável','Canal','Obrig.','Status','Ações'].map(h=>(
                    ...
                    <tr key={c.id} style={{ background:i%2===0?'#fff':'#fafafa', borderBottom:'1px solid #f0f0f0' }}>
                      ...
                      <td style={{ padding:'9px 12px' }}>
                        <div style={{ display:'flex', gap:6 }}>
                          ...
                          <button onClick={()=>{ editar(c); setTimeout(()=>setModalObrig(true),100) }} style={{ padding:'4px 9px', borderRadius:6, background:'#F0F4FF', color:NAVY, border:'none', cursor:'pointer', fontSize:11 }}>📋 Vínculos</button>
                        </div>
                      </td>
                    </tr>
                  )
            ...
            <div style={{ maxWidth:900, margin:'0 auto', background:'#fff', borderRadius:12, padding:24, border:'1px solid #e8e8e8' }}>
              ...
              {abaForm==='endereco' && (
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', gap:12 }}>
                  {[{k:'cep',l:'CEP',p:'00000-000',g:1},{k:'logradouro',l:'Logradouro',p:'Rua...',g:2},{k:'numero',l:'Número',p:'',g:1},{k:'complemento',l:'Complemento',p:'Apto...',g:1},{k:'bairro',l:'Bairro',p:'',g:1},{k:'cidade',l:'Cidade',p:'',g:1},{k:'estado',l:'Estado (UF)',p:'GO',g:1}].map(f2=>(
              ...
              {abaForm==='responsavel' && (
                <div>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
                    ...
                    <div style={{ display:'flex', gap:8 }}>
                      <button onClick={()=>setF('responsaveis',[...(form.responsaveis||[]),{tipo:'PF',nome:'',cpf_cnpj:'',cargo:'',email:'',whatsapp:''}])} style={{ display:'flex', alignItems:'center', gap:5, padding:'6px 12px', borderRadius:7, background:NAVY, color:'#fff', fontSize:12, fontWeight:600, border:'none', cursor:'pointer' }}>+ PF</button>
                      <button onClick={()=>setF('responsaveis',[...(form.responsaveis||[]),{tipo:'PJ',nome:'',cpf_cnpj:'',cargo:'',email:'',whatsapp:''}])} style={{ display:'flex', alignItems:'center', gap:5, padding:'6px 12px', borderRadius:7, background:GOLD, color:NAVY, fontSize:12, fontWeight:600, border:'none', cursor:'pointer' }}>+ PJ</button>
                    </div>
                  </div>
                    ...
                    <div key={ri} style={{ marginBottom:14, padding:'14px 16px', borderRadius:10, border:`1px solid ${resp.tipo==='PJ'?GOLD:'#3b82f6'}30`, background:resp.tipo==='PJ'?GOLD+'06':'#EBF5FF' }}>
                      ...
                      <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr 1fr', gap:10, marginBottom:8 }}>
                        <div>
                          ...
                          <input value={resp.nome} onChange={e=>{const r=[...form.responsaveis];r[ri]={...r[ri],nome:e.target.value};setF('responsaveis',r)}} style={inp} />
                        </div>
                        <div>
                          <label style={{ fontSize:10, color:'#888', fontWeight:600, display:'block', marginBottom:3 }}>{resp.tipo==='PJ'?'CNPJ':'CPF'}</label>
                          <input value={resp.cpf_cnpj} onChange={e=>{const r=[...form.responsaveis];r[ri]={...r[ri],cpf_cnpj:e.target.value};setF('responsaveis',r)}} style={inp} />
                        </div>
                        <div>
                          <label style={{ fontSize:10, color:'#888', fontWeight:600, display:'block', marginBottom:3 }}>Cargo</label>
                          <input value={resp.cargo} onChange={e=>{const r=[...form.responsaveis];r[ri]={...r[ri],cargo:e.target.value};setF('responsaveis',r)}} style={inp} />
                        </div>
                      </div>
                      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                        <div>
                          <label style={{ fontSize:10, color:'#888', fontWeight:600, display:'block', marginBottom:3 }}>E-mail</label>
                          <input type="email" value={resp.email} onChange={e=>{const r=[...form.responsaveis];r[ri]={...r[ri],email:e.target.value};setF('responsaveis',r)}} style={inp} />
                        </div>
                        <div>
                          <label style={{ fontSize:10, color:'#888', fontWeight:600, display:'block', marginBottom:3 }}>WhatsApp</label>
                          <input value={resp.whatsapp} onChange={e=>{const r=[...form.responsaveis];r[ri]={...r[ri],whatsapp:e.target.value};setF('responsaveis',r)}} style={inp} />
                        </div>
                      </div>
                    </div>
                  ))}
                    ...
                    return (
                      <div key={ci} style={{ marginBottom:10, padding:'14px 16px', borderRadius:10, border:`1px solid ${isPrincipal?NAVY+'30':'#e8e8e8'}`, background:isPrincipal?'#f8f9ff':'#fff' }}>
                        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
                          ...
                          {!isPrincipal && <button onClick={()=>setF('contatos',form.contatos.filter((_,i)=>i!==ci))} style={{ padding:'3px 8px', borderRadius:6, background:'#FEF2F2', color:'#dc2626', border:'none', cursor:'pointer', fontSize:11 }}>🗑️</button>}
                        </div>
                        <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr 1fr', gap:10, marginBottom:8 }}>
                          <div>
                            <label style={{ fontSize:10, color:'#888', fontWeight:600, display:'block', marginBottom:3 }}>Nome</label>
                            <input value={ct.nome} onChange={e=>{const cs=[...form.contatos];cs[ci]={...cs[ci],nome:e.target.value};setF('contatos',cs)}} style={inp} />
                          </div>
                          <div>
                          ...
                          <div>
                            <label style={{ fontSize:10, color:'#888', fontWeight:600, display:'block', marginBottom:3 }}>Cargo</label>
                            <input value={ct.cargo||''} onChange={e=>{const cs=[...form.contatos];cs[ci]={...cs[ci],cargo:e.target.value};setF('contatos',cs)}} style={inp} />
                          </div>
                        </div>
                        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:10 }}>
                          <div>
                            <label style={{ fontSize:10, color:'#888', fontWeight:600, display:'block', marginBottom:3 }}>E-mail</label>
                            <input type="email" value={ct.email||''} onChange={e=>{const cs=[...form.contatos];cs[ci]={...cs[ci],email:e.target.value};setF('contatos',cs)}} style={inp} />
                          </div>
                          <div>
                            <label style={{ fontSize:10, color:'#888', fontWeight:600, display:'block', marginBottom:3 }}>WhatsApp</label>
                            <div style={{ display:'flex', gap:5 }}>
                              <input value={ct.whatsapp||''} onChange={e=>{const cs=[...form.contatos];cs[ci]={...cs[ci],whatsapp:e.target.value};setF('contatos',cs)}} style={{ ...inp, flex:1 }} />
                              {ct.whatsapp && <a href={`https://wa.me/${ct.whatsapp.replace(/\D/g,'')}`} target="_blank" rel="noreferrer" style={{ display:'flex', alignItems:'center', padding:'0 9px', borderRadius:7, background:'#22c55e', color:'#fff', fontSize:16, textDecoration:'none', flexShrink:0 }}>💬</a>}
                            </div>
                          </div>
                          <div>
                            <label style={{ fontSize:10, color:'#888', fontWeight:600, display:'block', marginBottom:3 }}>Telefone</label>
                            <input value={ct.telefone||''} onChange={e=>{const cs=[...form.contatos];cs[ci]={...cs[ci],telefone:e.target.value};setF('contatos',cs)}} style={inp} />
                          </div>
                        </div>
                      </div>
                    )
              ...
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', paddingTop:14, marginTop:14, borderTop:'1px solid #f0f0f0' }}>
                ...
                <div style={{ display:'flex', gap:8 }}>
                  {/* Botão editar vínculos de obrigações */}
                  <button onClick={()=>setModalObrig(true)} style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 18px', borderRadius:8, background:'#F0F4FF', color:NAVY, fontSize:13, fontWeight:700, border:`2px solid ${NAVY}30`, cursor:'pointer' }}>
                    ✏️ Editar Vínculos de Obrigações
                    <span style={{ background:form.obrigacoes_vinculadas.length>0?NAVY:'#ccc', color:'#fff', borderRadius:10, padding:'1px 8px', fontSize:12, fontWeight:700 }}>
                      {form.obrigacoes_vinculadas.length}
                    </span>
                  </button>
                  {editId && (
                    <button onClick={()=>setModalGerar(true)} style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 14px', borderRadius:8, background:'#FFFBF0', color:'#854D0E', fontSize:12, fontWeight:700, border:`1px solid ${GOLD}`, cursor:'pointer' }}>
                      📅 Gerar Tarefas
                    </button>
                  )}
      ...
      {modalGerar && editId && (
        <GerarObrigacoes
          cliente={{...form, id:editId}}
          onClose={()=>setModalGerar(false)}
          onGerado={(qtd)=>{ setModalGerar(false); alert(`${qtd} obrigações geradas em Entregas/Tarefas!`) }}
        />
      )}
            ...
            <div style={{ background:'#fff', borderRadius:14, width:'100%', maxWidth:680, maxHeight:'90vh', display:'flex', flexDirection:'column', boxShadow:'0 20px 60px rgba(0,0,0,0.2)' }}>
              ...
              <div style={{ padding:'8px 20px', borderBottom:'1px solid #f0f0f0', display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
                ...
                <div style={{ display:'flex', gap:6, marginLeft:'auto' }}>
                  <button onClick={()=>setF('obrigacoes_vinculadas',filtObrig.map(o=>o.id))} style={{ padding:'4px 10px', borderRadius:7, background:NAVY, color:'#fff', fontSize:11, fontWeight:600, border:'none', cursor:'pointer' }}>Selec. tudo ({filtObrig.length})</button>
              ...
              <div style={{ flex:1, overflowY:'auto', padding:'8px 20px' }}>
                {abaObrig==='lista' && (
                  filtObrig.length===0
                    ? <div style={{ textAlign:'center', padding:30, color:'#ccc', fontSize:13 }}>Nenhuma obrigação.</div>
                      ...
                      return (
                        <div key={dept} style={{ marginBottom:16 }}>
                          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8, paddingBottom:5, borderBottom:'1px solid #f0f0f0' }}>
                            ...
                            <span style={{ fontSize:11, color:'#aaa' }}>👤 {dr.responsavel}</span>
                            <span style={{ fontSize:10, color:'#bbb', marginLeft:'auto' }}>{lista.length} · {lista.filter(o=>form.obrigacoes_vinculadas.includes(o.id)).length} selecionadas</span>
                          </div>
                          {lista.map(o => {
                        ...
                        <div key={dept} style={{ marginBottom:16, borderRadius:12, border:'1px solid #e8e8e8', overflow:'hidden' }}>
                          <div style={{ padding:'12px 16px', background:selQtd>0?dr.cor?.bg||'#f5f5f5':'#fafafa', borderBottom:'1px solid #f0f0f0', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                            <div>
                              ...
                              <div style={{ fontSize:11, color:'#777' }}>👤 {dr.responsavel}</div>
                            </div>
                            <div style={{ textAlign:'right' }}>
                              <div style={{ fontSize:16, fontWeight:800, color:selQtd>0?dr.cor?.color||NAVY:'#ccc' }}>{selQtd}/{lista.length}</div>
                              <div style={{ display:'flex', gap:5, marginTop:4 }}>
                                <button onClick={()=>setF('obrigacoes_vinculadas',[...form.obrigacoes_vinculadas,...lista.map(o=>o.id).filter(id=>!form.obrigacoes_vinculadas.includes(id))])} style={{ padding:'3px 8px', borderRadius:6, background:NAVY, color:'#fff', fontSize:10, fontWeight:600, border:'none', cursor:'pointer' }}>Todos</button>
