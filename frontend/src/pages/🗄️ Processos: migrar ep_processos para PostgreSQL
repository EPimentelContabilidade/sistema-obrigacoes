...
function TabProcessos({ templates }) {
  const API_BASE = window.location.hostname === 'localhost' ? '/api/v1' : 'https://sistema-obrigacoes-production.up.railway.app/api/v1';
  ...
  const salvarProcessos = lista => { setProcessos(lista); epSet("ep_processos", lista); };
  ...
  const confirmar = (msg, onOk) => setConfirmModal({msg, onOk});
        ...
        <div style={{ flex:1,overflowY:"auto",padding:20,background:"#F8F9FA" }}>
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14 }}>
            ...
            <div style={{display:'flex',gap:5,alignItems:'center'}}>
              ...
              <button onClick={()=>confirmar('Excluir este processo permanentemente?',()=>{const l=processos.filter(p=>p.id!==selecionado.id);salvarProcessos(l);setSelecionado(null);})}
