...
const gerarRelatorioCliente = async (cli, resultados, historico) => {
  ...
  const W = 210, M = 15
  const NAVY = [27,42,74], GOLD = [197,165,90], GRAY = [80,80,80], LGRAY = [200,200,200]
  ...
  doc.text(`Por meio do sistema EPimentel — CNPJ do responsável: 22.939.803/0001-49`, M+6, y+17)
  ...
  doc.text('Dados Cadastrais da Matriz', M, y); y+=1
  ...
  campo('Situação', cnpjR.situacao_cadastral||'—', M+95, y)
  y+=6
  campo('Razão Social', cnpjR.razao_social||cli?.nome||'—', M, y, 170)
  y+=6
  campo('Natureza Jurídica', cnpjR.natureza_juridica||'—', M, y)
  campo('CNAE Principal', (cnpjR.atividade_principal||'—').substring(0,35), M+95, y)
  y+=6
  if (cnpjR.endereco) {
    const end = cnpjR.endereco
    campo('Endereço', `${end.logradouro||''}, ${end.numero||''} — ${end.municipio||''}/${end.uf||''}`, M, y, 170)
    y+=6
  }
  ...
  if (cnpjR.socios?.length > 0) {
    ...
    doc.text('Sócios e Administradores', M, y); y+=1
    doc.setDrawColor(...LGRAY); doc.line(M,y,W-M,y); y+=4
    doc.setFont('helvetica','normal'); doc.setTextColor(...GRAY)
    cnpjR.socios.slice(0,5).forEach(s => {
      doc.text(`• ${s.nome_socio||'—'}  —  Qualificação: ${s.qualificacao_socio||'—'}`, M+3, y); y+=5
    })
  }
  ...
  doc.setDrawColor(...LGRAY); doc.line(M,y,W-M,y); y+=5
  ...
  doc.text(
    semPendencias
      ? 'Não foram detectadas pendências/exigibilidades suspensas nos controles da Receita Federal e PGFN.'
      : 'Foram detectadas pendências. Verifique os detalhes abaixo.',
  ...
  if (histCli.length > 0) {
    doc.setFont('helvetica','bold'); doc.setTextColor(...NAVY); doc.setFontSize(9.5)
    doc.text('Histórico de Consultas', M, y); y+=1
    doc.setDrawColor(...LGRAY); doc.line(M,y,W-M,y); y+=4
    ...
    histCli.slice(0,6).forEach((h,i)=>{
      ...
      doc.text(h.status||'—', M+100, y+0.5)
      doc.setFont('helvetica','normal'); doc.setTextColor(...GRAY)
      doc.text((h.obs||'').substring(0,28), M+135, y+0.5)
      y+=6
    })
  }
  ...
  doc.text(`Documento gerado eletronicamente em ${hoje}  ·  Este documento não substitui certidões oficiais emitidas pela Receita Federal.`, W/2, rodY+11, {align:'center'})
...
const gerarPDFFiscal = async (historico, clientes, filtroCliente) => {
  ...
  doc.setFontSize(9); doc.text(`Emitido em: ${hoje}`, W-14, 11, {align:'right'})
...
const PORTAIS = [
  ...
  { id:'completo',   label:'Consulta Completa (todos)',  icon:'🔍', cor:NAVY,      bg:'#f0f4ff', auto:true,  desc:'CNPJ + Simples + PGFN' },
...
export default function RelatorioFiscal() {
  ...
  const consultarAuto = async (portal) => {
    ...
    try {
      ...
      if (portal.id==='cnpj_dados') {
        ...
        dados={razao_social:d.razao_social,situacao_cadastral:d.descricao_situacao_cadastral,natureza_juridica:d.descricao_natureza_juridica,atividade_principal:d.cnae_fiscal_descricao,capital_social:d.capital_social,socios:d.qsa||[],endereco:{logradouro:d.logradouro,numero:d.numero,municipio:d.municipio,uf:d.uf},consultado_em:new Date().toISOString()}
      } else if (portal.id==='simples') {
        const d=await buscarBrasilAPI(cnpj)
        dados={regime:d.opcao_pelo_mei?'MEI':d.opcao_pelo_simples?'Simples Nacional':'Não optante',optante_simples:d.opcao_pelo_simples,optante_mei:d.opcao_pelo_mei,data_opcao_simples:d.data_opcao_pelo_simples,consultado_em:new Date().toISOString()}
      } else if (portal.id==='pgfn') {
        try { const r=await fetch(`https://www.regularize.pgfn.gov.br/api/v1/situacao-devedores/${cnpj}`); if(r.ok){dados=await r.json()}else{dados={mensagem:'Consulte www.regularize.pgfn.gov.br'}} } catch { dados={mensagem:'Consulte www.regularize.pgfn.gov.br'} }
        dados.consultado_em=new Date().toISOString()
      } else if (portal.id==='completo') {
        const d=await buscarBrasilAPI(cnpj)
        const ok=d.descricao_situacao_cadastral?.toLowerCase().includes('ativa')
        dados={razao_social:d.razao_social,situacao_cadastral:d.descricao_situacao_cadastral,regime:d.opcao_pelo_mei?'MEI':d.opcao_pelo_simples?'Simples Nacional':'Outro',socios:d.qsa||[],resumo:{situacao_geral:ok?'Regular':'Verificar situação',pendencias:ok?[]:[`Situação: ${d.descricao_situacao_cadastral}`]},consultado_em:new Date().toISOString()}
      }
  ...
  const RenderResultado = ({portalId}) => {
    ...
    if(r.mensagem) campos.push(['Info',r.mensagem])
      ...
      <div style={{marginTop:8,padding:'10px 12px',borderRadius:8,background:cor,border:'1px solid #e8e8e8',fontSize:11}}>
        {campos.map(([k,v])=>(
          <div key={k} style={{display:'flex',gap:8,marginBottom:3}}>
            <span style={{color:'#888',minWidth:120,fontWeight:600}}>{k}:</span>
            <span style={{color:NAVY,fontWeight:500}}>{v}</span>
          </div>
        ))}
        {r.socios?.length>0&&<div style={{marginTop:4,color:'#555'}}>Sócios: {r.socios.map(s=>s.nome_socio).join(', ')}</div>}
        <div style={{marginTop:4,color:'#aaa',fontSize:10}}>Consultado: {new Date(r.consultado_em).toLocaleString('pt-BR')}</div>
      </div>
    )
  }
                      ...
                      <div style={{padding:'14px 16px',display:'flex',alignItems:'flex-start',gap:12}}>
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:2}}>
                            ...
                            {p.auto?<span style={{fontSize:9,padding:'1px 6px',borderRadius:5,background:'#EBF5FF',color:'#1D6FA4',fontWeight:700}}>⚡ AUTO</span>:<span style={{fontSize:9,padding:'1px 6px',borderRadius:5,background:'#F3EEFF',color:'#6B3EC9',fontWeight:700}}>🔐 MANUAL</span>}
                          </div>
                        ...
                        <div style={{display:'flex',gap:6,flexShrink:0}}>
                          {p.auto?<button onClick={()=>consultarAuto(p)} disabled={loading} style={{display:'flex',alignItems:'center',gap:5,padding:'6px 12px',borderRadius:8,background:'#EBF5FF',color:'#1D6FA4',fontWeight:700,fontSize:12,border:'1px solid #1D6FA430',cursor:'pointer'}}>{loading?<Loader size={11} style={{animation:'spin 1s linear infinite'}}/>:<Zap size={11}/>} {loading?'Buscando...':'Buscar'}</button>
                          :<button onClick={()=>window.open(p.url,'_blank')} style={{display:'flex',alignItems:'center',gap:5,padding:'6px 12px',borderRadius:8,background:p.bg,color:p.cor,fontWeight:700,fontSize:12,border:`1px solid ${p.cor}30`,cursor:'pointer'}}><ExternalLink size={11}/> Acessar</button>}
      ...
      {aba==='dashboard'&&(
        <div style={{flex:1,overflowY:'auto',padding:'16px 20px',background:'#f8f9fb'}}>
          ...
          <div style={{marginBottom:12}}>
            <input value={filtroCliDash} onChange={e=>setFiltroCliDash(e.target.value)} placeholder="Filtrar cliente..." style={{...inp,maxWidth:300,fontSize:12}}/>
          </div>
          <div style={{background:'#fff',borderRadius:12,border:'1px solid #e8e8e8',overflow:'hidden'}}>
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
              <thead><tr style={{background:NAVY}}>{['Cliente','CNPJ','Regime','Procuração','Certificado','Última Consulta','Situação','Ações'].map(h=><th key={h} style={{padding:'10px 14px',textAlign:'left',fontSize:10,fontWeight:700,color:'#fff',textTransform:'uppercase',whiteSpace:'nowrap'}}>{h}</th>)}</tr></thead>
              <tbody>
          ...
          <div style={{maxWidth:700,margin:'0 auto',background:'#fff',borderRadius:12,padding:28,border:'1px solid #e8e8e8'}}>
            ...
            <div style={{marginBottom:20,padding:'14px 16px',borderRadius:10,background:'#FEF2F2',border:'1px solid #fca5a5'}}>
              ...
              <div style={{display:'flex',gap:10,alignItems:'center',flexWrap:'wrap'}}>
                <select value={filtroCliDash} onChange={e=>setFiltroCliDash(e.target.value)} style={{...inp,maxWidth:280,cursor:'pointer',fontSize:12}}>
                  <option value="">Selecione o cliente...</option>
                  {clientes.filter(c=>c.ativo!==false).map(c=><option key={c.id} value={c.nome}>{c.nome}</option>)}
                </select>
                <button onClick={()=>{
                  const cli=clientes.find(c=>c.nome===filtroCliDash)
                  const res={}
                  historico.filter(h=>String(h.cliente_id)===String(cli?.id)).forEach(h=>{if(!res[h.relatorio_id])res[h.relatorio_id]=h})
                  gerarRelatorioCliente(cli,res,historico)
                }} disabled={!filtroCliDash}
                  style={{display:'flex',alignItems:'center',gap:8,padding:'9px 20px',borderRadius:9,background:filtroCliDash?'#dc2626':'#ccc',color:'#fff',fontWeight:700,fontSize:13,border:'none',cursor:filtroCliDash?'pointer':'default'}}>
                  <Download size={14}/> Gerar Diagnóstico Fiscal
                </button>
              </div>
            </div>
              ...
              <div style={{marginBottom:16}}>
                ...
                {certServidor.map(c=>(
                  <div key={c.nome} style={{display:'flex',alignItems:'center',gap:10,padding:'8px 12px',borderRadius:8,background:'#F0FDF4',border:'1px solid #bbf7d0',marginBottom:6}}>
                    ...
                    <div style={{flex:1}}><div style={{fontSize:12,fontWeight:600,color:NAVY}}>{c.nome}</div><div style={{fontSize:10,color:'#aaa'}}>{c.tamanho_kb} KB</div></div>
                    {cliSel&&<button onClick={()=>acessarEcacAutomatico(c.caminho)} disabled={ecacRodando} style={{display:'flex',alignItems:'center',gap:5,padding:'5px 12px',borderRadius:7,background:'#6B3EC9',color:'#fff',fontWeight:700,fontSize:11,border:'none',cursor:'pointer'}}>{ecacRodando?<><Loader size={11} style={{animation:'spin 1s linear infinite'}}/> Conectando...</>:'🤖 Usar no e-CAC'}</button>}
                  </div>
                ))}
                {ecacResultado&&<div style={{marginTop:8,padding:'10px 14px',borderRadius:8,background:ecacResultado.status==='conectado'?'#F0FDF4':'#FEF2F2',fontSize:12}}>{ecacResultado.status==='conectado'?<><b style={{color:'#16a34a'}}>✓ Conectado!</b> {ecacResultado.pagina}</>:<><b style={{color:'#dc2626'}}>✗ Erro:</b> {ecacResultado.mensagem||ecacResultado.detail}</>}</div>}
              </div>
            )}
            <div style={{marginBottom:14}}>
              <div style={{fontSize:11,fontWeight:700,color:'#888',textTransform:'uppercase',marginBottom:8}}>Enviar certificado .pfx</div>
              <div style={{padding:'14px 16px',borderRadius:10,background:GOLD+'08',border:`2px dashed ${GOLD}`}}>
                <label style={{display:'flex',alignItems:'center',gap:8,cursor:'pointer',marginBottom:10}}>
                  <div style={{padding:'8px 16px',borderRadius:8,background:NAVY,color:'#fff',fontWeight:700,fontSize:12}}>📁 Selecionar .pfx</div>
                  <span style={{fontSize:12,color:'#888'}}>{certUpload?certUpload.name:'Nenhum arquivo'}</span>
                ...
                <div style={{marginBottom:10}}><label style={{fontSize:11,color:'#888',fontWeight:600,display:'block',marginBottom:4}}>Senha</label><input type="password" value={certSenhaUp} onChange={e=>setCertSenhaUp(e.target.value)} placeholder="Senha do .pfx" style={{...inp,fontSize:13}}/></div>
                <button onClick={fazerUploadCert} disabled={!certUpload||uploadStatus==='enviando'} style={{display:'flex',alignItems:'center',gap:6,padding:'8px 18px',borderRadius:8,background:certUpload?'#22c55e':'#ccc',color:'#fff',fontWeight:700,fontSize:13,border:'none',cursor:certUpload?'pointer':'default'}}>{uploadStatus==='enviando'?<><Loader size={13} style={{animation:'spin 1s linear infinite'}}/> Enviando...</>:'⬆ Enviar para o servidor'}</button>
                {uploadStatus.startsWith('ok')&&<div style={{marginTop:8,fontSize:11,color:'#16a34a',fontWeight:600}}>✓ Certificado enviado!</div>}
                {uploadStatus==='erro'&&<div style={{marginTop:8,fontSize:11,color:'#dc2626'}}>✗ Erro ao enviar.</div>}
              </div>
            </div>
