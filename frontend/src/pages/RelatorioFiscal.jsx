...
const GOLD  = '#C5A55A'
const API   = '/api/v1'
// URL absoluta do backend para uploads (necessário em produção Railway)
...
export default function RelatorioFiscal() {
  ...
  const carregarCertsServidor = async () => {
    try {
      const r = await fetch(`${BACKEND}/consulta-fiscal/certificados/listados`)
  ...
  const acessarEcacAutomatico = async (portal, certPath) => {
    ...
    try {
      const r = await fetch(`${BACKEND}/consulta-fiscal/ecac/autenticar`, {
        method:'POST',
