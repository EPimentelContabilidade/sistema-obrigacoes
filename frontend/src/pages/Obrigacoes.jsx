...
export default function Obrigacoes() {
  ...
  useEffect(()=>{ fetch(`${API}/clientes/`).then(r=>r.ok?r.json():{}).then(d=>setClientes(d.clientes||d||[])).catch(()=>{}) },[])

  const nova = () => { setForm(FORM0); setEditandoId(null); setAba('cadastro') }
  const editar = (o) => {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'calc(100vh - 44px)', fontFamily:'Inter, system-ui, sans-serif' }}>
      ...
      <div style={{ background:'#fff', borderBottom:'1px solid #e8e8e8', display:'flex', alignItems:'center', padding:'0 14px' }}>
        {[['lista',`Obrigações (${obrigacoes.filter(o=>o.ativa).length} ativas)`],['cadastro',editandoId?'Editar Obrigação':'Cadastro de obrigação']].map(([id,label])=>(
          <button key={id} onClick={()=>setAba(id)} style={{ padding:'10px 14px', fontSize:13, fontWeight:aba===id?700:400, color:aba===id?NAVY:'#999', background:'none', border:'none', borderBottom:aba===id?`2px solid ${GOLD}`:'2px solid transparent', cursor:'pointer' }}>{label}</button>
        ))}
                    ...
                    <tr key={o.id} style={{ background:o.ativa?(i%2===0?'#fff':'#fafafa'):'#f5f5f5', borderBottom:'1px solid #f0f0f0', opacity:o.ativa?1:0.6 }}>
                      ...
                      <td style={{ padding:'8px 10px' }}>
                        <div style={{ display:'flex', gap:4 }}>
                          ...
                          <button onClick={()=>abrirEmp(o)} title="Adicionar empresa" style={{ padding:'4px 8px', borderRadius:6, background:GOLD+'20', color:GOLD, border:'none', cursor:'pointer', fontSize:11 }}>🏢</button>
      ...
      {/* ── CADASTRO ── */}
      {aba==='cadastro' && (
        <div style={{ flex:1, overflowY:'auto', padding:16, background:'#f8f9fb' }}>
          <div style={{ maxWidth:1100, margin:'0 auto', background:'#fff', borderRadius:12, padding:22, border:'1px solid #e8e8e8' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'3fr 1fr 2fr 1fr', gap:10, marginBottom:14 }}>
              ...
              <div>
                <L tip="Abreviação para relatórios (máx 12 char)">Mininome</L>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:10, marginBottom:14 }}>
              <div><L tip="Quantos dias antes o sistema notifica o responsável">Lembrar responsável quantos dias antes?</L>
              ...
              <div><L tip="O que fazer quando o prazo cair em dia não-útil">Prazos fixos em dias não-úteis</L>
              ...
              <div><L tip="Sábado conta como dia útil para cálculo de prazos?">Sábado é útil?</L>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(5, 1fr)', gap:10, marginBottom:14 }}>
              <div><L tip="A qual período de competência esta obrigação se refere">Competências referentes a</L>
              ...
              <div><L tip="Se Sim, a entrega só é feita após o Robô processar o documento">Exigir Robô?</L>
              ...
              <div><L tip="Alertar quando a guia não for visualizada pelo cliente">Alerta guia ñ-lida?</L>
            ...
            <div style={{ marginBottom:14, padding:'10px 14px', borderRadius:8, border:'1px solid #e8e8e8', background:'#fafafa', display:'flex', alignItems:'center', gap:14 }}>
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:11, color:'#aaa' }}>Ao marcar como entregue, notifica automaticamente o cliente via WhatsApp</div>
              </div>
            ...
            {/* Tributação */}
            <div style={{ marginBottom:14 }}>
              <L tip="Regimes tributários que utilizam esta obrigação. Ao cadastrar cliente, as obrigações do seu regime serão sugeridas automaticamente.">Tributação (Regimes que usam esta obrigação)</L>
            ...
            <div style={{ marginBottom:14 }}>
              ...
              <textarea value={form.comentario_padrao} onChange={e=>setF('comentario_padrao',e.target.value)} placeholder="Comentário padrão..." style={{ ...inp, height:70, resize:'vertical', fontFamily:'inherit' }} />
            </div>
              ...
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
                <div>
                  <div style={{ fontSize:12, fontWeight:700, color:NAVY }}>
                    Empresas que precisam entregar essa obrigação [{(form.empresas||[]).length} empresa(s)]
                  </div>
                  <div style={{ fontSize:10, color:'#aaa', marginTop:2 }}>
                    <span style={{ cursor:'pointer', color:'#3b82f6' }} onClick={()=>alert('Lista de empresas: '+(form.empresas||[]).join(', ')||'Nenhuma')}>Clique para listar</span>
                  </div>
                </div>
                <button onClick={()=>abrirEmp({id:editandoId||'_novo',...form})} style={{ display:'flex', alignItems:'center', gap:5, padding:'6px 14px', borderRadius:8, background:GOLD, color:NAVY, fontSize:12, fontWeight:700, border:'none', cursor:'pointer' }}>
                  <Plus size={12} /> Adicionar empresa a essa obrigação
                </button>
              </div>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', paddingTop:14, borderTop:'1px solid #f0f0f0' }}>
              <div style={{ display:'flex', gap:8 }}>
                <button onClick={()=>setAba('lista')} style={{ display:'flex', alignItems:'center', gap:5, padding:'8px 14px', borderRadius:8, background:'#6B3EC9', color:'#fff', fontSize:12, fontWeight:600, border:'none', cursor:'pointer' }}>↩ Retro</button>
                <button style={{ display:'flex', alignItems:'center', gap:5, padding:'8px 14px', borderRadius:8, background:'#3b82f6', color:'#fff', fontSize:12, fontWeight:600, border:'none', cursor:'pointer' }}>📋 Avulsa</button>
              </div>
              <div style={{ display:'flex', gap:8 }}>
                <button onClick={salvar} disabled={!form.nome} style={{ display:'flex', alignItems:'center', gap:5, padding:'8px 20px', borderRadius:8, background:form.nome?'#22c55e':'#ccc', color:'#fff', fontWeight:700, fontSize:13, border:'none', cursor:form.nome?'pointer':'default' }}><Save size={13}/> Salvar</button>
                <button onClick={nova} style={{ display:'flex', alignItems:'center', gap:5, padding:'8px 14px', borderRadius:8, background:NAVY, color:'#fff', fontWeight:700, fontSize:12, border:'none', cursor:'pointer' }}><Plus size={13}/> Nova</button>
                <button onClick={()=>setAba('lista')} style={{ display:'flex', alignItems:'center', gap:5, padding:'8px 14px', borderRadius:8, background:'#f59e0b', color:'#fff', fontWeight:700, fontSize:12, border:'none', cursor:'pointer' }}><X size={13}/> Voltar</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.45)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:200 }}>
          <div style={{ background:'#fff', borderRadius:14, width:'100%', maxWidth:520, maxHeight:'85vh', display:'flex', flexDirection:'column', boxShadow:'0 24px 60px rgba(0,0,0,0.2)' }}>
            <div style={{ padding:'14px 20px', borderBottom:'1px solid #f0f0f0', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <div>
                <div style={{ fontWeight:700, color:NAVY, fontSize:14 }}>Adicionar empresa a essa obrigação</div>
            ...
            {/* Seleção por tributação */}
            <div style={{ padding:'6px 20px', borderBottom:'1px solid #f0f0f0', display:'flex', gap:5, flexWrap:'wrap', alignItems:'center' }}>
              <span style={{ fontSize:10, color:'#aaa' }}>Por regime:</span>
              {TRIBUTACOES.slice(0,5).map(t=>(
                <button key={t} style={{ padding:'2px 8px', borderRadius:10, fontSize:10, background:'#f5f5f5', color:'#666', border:'1px solid #e0e0e0', cursor:'pointer' }}>{t}</button>
              ))}
            </div>
            <div style={{ flex:1, overflowY:'auto', padding:'8px 20px' }}>
              {clientes.length===0 ? (
                <div style={{ padding:20, textAlign:'center', color:'#ccc', fontSize:13 }}>
                  ...
                  <div style={{ fontSize:11, marginTop:6 }}>Cadastre clientes na aba "Clientes" para vinculá-los.</div>
                </div>
              ) : clientes.filter(c=>c.nome.toLowerCase().includes(buscaEmp.toLowerCase())||(c.cnpj||'').includes(buscaEmp)).map(c=>{
                const s=empSel.includes(c.nome)
                return (
                  <label key={c.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 0', cursor:'pointer', borderBottom:'1px solid #f5f5f5' }}>
                    <input type="checkbox" checked={s} onChange={()=>setEmpSel(v=>s?v.filter(x=>x!==c.nome):[...v,c.nome])} style={{ width:15, height:15, accentColor:NAVY }} />
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:13, fontWeight:600, color:NAVY }}>{c.nome}</div>
                      <div style={{ fontSize:11, color:'#aaa', marginTop:1 }}>{c.cnpj} · {c.regime||'Regime não informado'}</div>
                    </div>
            ...
            <div style={{ padding:'12px 20px', borderTop:'1px solid #f0f0f0', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <span style={{ fontSize:11, color:'#aaa' }}>{empSel.length} empresa(s) selecionada(s)</span>
              <div style={{ display:'flex', gap:8 }}>
