...
class ComunicadoCreate(BaseModel):
    titulo:       str
    conteudo:     str
    urgencia:     str = "normal"          # baixa | normal | alta | muito_urgente
    departamento: str = "Geral"
    responsavel:  str = ""
    canal:        str = "email"           # email | whatsapp | ambos | interno
    cliente_ids:  List[int] = []
    processo_ids: List[int] = []
    emails_extra: List[str] = []          # e-mails adicionais (fora do sistema)
    usa_dominio_proprio: bool = False     # True = usa SMTP_HOST, False = Gmail
...
# ── Template HTML do comunicado ────────────────────────────────────────────────

def gerar_html_comunicado(titulo: str, conteudo: str, urgencia: str,
                           departamento: str, responsavel: str,
                           assinatura: str, destinatario_nome: str = "") -> str:
...
<div style="max-width:600px;margin:0 auto;padding:24px 16px;">
  <div style="background:linear-gradient(135deg,{NAVY},{NAVY}cc);border-radius:14px 14px 0 0;padding:24px 32px;">
    <div style="display:flex;align-items:center;gap:12px;">
      ...
      <span style="margin-left:auto;background:{urg['bg']};color:{urg['cor']};padding:4px 12px;border-radius:12px;font-size:11px;font-weight:800;border:1px solid {urg['cor']}33;">
        {urg['emoji']} {urg['label']}
      </span>
    </div>
  </div>
  <div style="background:#fff;padding:28px 32px;border-left:1px solid #e8e8e8;border-right:1px solid #e8e8e8;">
    <h2 style="color:{NAVY};margin:0 0 16px;font-size:18px;font-weight:800;">{titulo}</h2>
...
async def enviar_email_comunicado(para: str, nome: str, assunto: str,
                                   html: str, smtp_cfg: dict) -> bool:
...
async def analisar_comunicado_ia(titulo: str, conteudo: str, dias: int,
                                  departamento: str, responsavel: str) -> str:
    """Usa Claude para gerar alerta e sugestão de ação para comunicados atrasados."""
                ...
                json={
                    ...
                    "messages": [{
                        ...
                        "content": (
                            ...
                            "Gere um alerta profissional e urgente (máximo 4 linhas) para o responsável, "
                            "sugerindo uma ação imediata. Use emojis. Seja direto."
                        )
                    }]
                })
            data = r.json()
...
@router.post("/criar")
async def criar_comunicado(payload: ComunicadoCreate,
    ...
    async def _enviar():
        ...
        destinatarios = []  # (email, nome, telefone)
        ...
        for dest in destinatarios:
            if payload.canal in ("email", "ambos") and dest["email"]:
                html = gerar_html_comunicado(payload.titulo, payload.conteudo,
                    payload.urgencia, payload.departamento, payload.responsavel,
                    assinatura, dest["nome"])
            ...
            if payload.canal in ("whatsapp","ambos") and dest["tel"]:
                txt = (f"{urg['emoji']} *{payload.titulo}*\n"
                       f"_{urg['label']}_ | {payload.departamento}\n\n"
...
@router.get("/listar")
async def listar(urgencia: str = None, departamento: str = None,
...
async def responder(comunicado_id: int, payload: ComunicadoResposta,
                     db: AsyncSession = Depends(get_db)):
    ...
    respostas.append({"texto": payload.resposta, "respondente": payload.respondente,
                       "data": datetime.now().isoformat()})
    await db.execute(text("""
        UPDATE comunicados SET respostas=:resp, status='respondido', atualizado_em=datetime('now','localtime')
        WHERE id=:id
...
async def encerrar(comunicado_id: int, db: AsyncSession = Depends(get_db)):
    await db.execute(text("""
        UPDATE comunicados SET status='encerrado', encerrado_em=datetime('now','localtime')
        WHERE id=:id
    """), {"id": comunicado_id})
    await db.commit()
    return {"ok": True}
...
async def verificar_atrasados(background: BackgroundTasks, db: AsyncSession = Depends(get_db)):
    """Verifica comunicados >30 dias sem resposta e aciona IA Claude para gerar alertas."""
    await init_tables(db)
    r = await db.execute(text("""
    ...
    async def _processar():
        for com in atrasados:
            ...
            alerta = await analisar_comunicado_ia(
                com["titulo"], com["conteudo"] or "", dias,
                com["departamento"], com["responsavel"] or "")
            await db.execute(text("""
                UPDATE comunicados SET alerta_ia=:alerta, atualizado_em=datetime('now','localtime')
                WHERE id=:id
            """), {"alerta": alerta, "id": com["id"]})
            # Notificar responsável por WA
            if com.get("responsavel"):
                try:
                    from models import Usuario
                    ru = await db.execute(select(Usuario).where(
                        (Usuario.nome.ilike(f"%{com['responsavel']}%"))))
                    u = ru.scalar_one_or_none()
                    if u and getattr(u, "telefone", None):
                        await enviar_wa(u.telefone,
                            f"🚨 *ALERTA SISTEMA EPimentel*\n\n{alerta}")
                except Exception: pass
        await db.commit()
...
async def get_smtp(db: AsyncSession = Depends(get_db)):
    ...
    cfg.pop("pass", None)  # nunca retornar senha
    return cfg


@router.post("/config-smtp")
async def salvar_smtp(config: dict, db: AsyncSession = Depends(get_db)):
    """Salva configuração de SMTP personalizado (domínio próprio)."""
    await init_tables(db)
    await db.execute(text("""
